...
// Create new PDF
   ...
   // ----- ------------------ ------------ Savoinor  TVA  ------------------------ --->
    $sql = "SELECT * FROM orders INNER JOIN client ON id_client = client_id WHERE order_date >= '$start_date' AND order_date <= '$end_date' and order_status = 1 AND factureTva=11 GROUP BY numeroFacture";
...
// ----- ------------------ ------------ Savoinor Sans TVA  ------------------------ --->
     $sql21 = "SELECT * FROM orders INNER JOIN client ON id_client = client_id WHERE order_date >= '$start_date' AND order_date <= '$end_date' and order_status = 1 AND factureTva=21";
    $query21 = $connect->query($sql21);
    $totalHTVA21 =0;
	$totalTVA21 =0;
	$totalTVAC21 =0;
// ----- ------------------ ------------ Liquids  TVA  ------------------------ --->
    $sql12 = "SELECT * FROM orders INNER JOIN client ON id_client = client_id WHERE order_date >= '$start_date' AND order_date <= '$end_date' and order_status = 1 AND factureTva=12";
    $query12 = $connect->query($sql12);
    $totalHTVA12 =0;
	$totalTVA12 =0;
	$totalTVAC12 =0;
// ----- ------------------ ------------ Liquids Sans TVA  ------------------------ --->
    $sql22 = "SELECT * FROM orders INNER JOIN client ON id_client = client_id WHERE order_date >= '$start_date' AND order_date <= '$end_date' and order_status = 1 AND factureTva=22";
...
$table .='<table style="" class="border">
        ...
        while ($result = $query->fetch_assoc()) { 
            ...
            $orderItemSql = "SELECT order_item.product_id, order_item.rate, order_item.quantity, order_item.total, product.product_name FROM order_item
                ...
                <td rowspan="'. $lineRow.'">'.$result['nom_client'].' '.$result['prenom_client'].' - '.$result['telephone_client'].'</td>';
