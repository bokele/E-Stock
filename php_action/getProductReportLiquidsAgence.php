...
// ----- ------------------ ------------ Liquids  TVA  ------------------------ --->
    $sql12 = "SELECT * FROM produit_history_agence INNER JOIN product ON produit_history_agence.product_id = product.product_id  WHERE date(produit_history_agence.date_add) >= date('$start_date') AND date(produit_history_agence.date_add) <= date('$end_date') AND product.brand_id = 2  AND  agenceId = $productStatusClient GROUP BY produit_history_agence.product_id";
...
<table style="margin-top: 30px;" class="border">
        ...
        while ($result = $query12->fetch_assoc()) { 
         	...
         	$sql122 = "SELECT * FROM produit_history_agence  WHERE date(produit_history_agence.date_add) >= date('$start_date') AND date(produit_history_agence.date_add) <= date('$end_date') AND product_id = $productId  AND  agenceId = $productStatusClient";
