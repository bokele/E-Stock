...
// ----- ------------------ ------------ Savonor  TVA  ------------------------ --->
    $sql = "SELECT * FROM produit_history INNER JOIN product ON produit_history.product_id = product.product_id  WHERE date(produit_history.date_add) >= date('$start_date') AND date(produit_history.date_add) <= date('$end_date') AND product.brand_id = 1 GROUP BY produit_history.product_id";
...
<?php $table ="";
$table .= '
<p> <h2 align="center"><u><b>Produits Savonor du '. $startDate.' au '. $endDate.'</b></u></h2></p>
